...
function getListBill(idShop) {
  return new Promise((reslove, reject) => {
    pool.query(
      "SELECT * FROM bill WHERE idShop = ? ORDER BY dateCreate DESC",
      [idShop],
      (err, rows) => {
        if (err) {
          reject(err);
...
function insertBill(value) {
  ...
  return new Promise((reslove, reject) => {
    ...
    pool.query(
      "INSERT INTO bill( name, status, totalPrice, idSeller, dateCreate, discount, idShop, description) VALUES(?,?,?,?,?,?,?,?)",
      [
        value["name"],
        value["status"],
        value["totalPrice"],
      ...
      (err, rows) => {
        if (err) {
          ...
          pool.query(
            "Select idShop,idBill, status from bill where idSeller= ? ORDER BY idBill DESC LIMIT 1",
            [value["idSeller"]],
            (err, rows) => {
              if (err) {
                reject(err);
                ...
                //   value["listMer"].forEach((element) => {
                pool.query(
                  "INSERT INTO refbillmerchandise(barcode, idShop, count, idBill, status)  VALUES ? ",
                  [listMer],
                  (err, rows) => {
                    if (err) {
...
function updateBill(value) {
  return new Promise((reslove, reject) => {
    pool.query(
      "UPDATE bill SET name=?,idSeller=?,discount=?,idShop=? WHERE idBill= ?",
      [
        value["name"],
        value["idSeller"],
        value["discount"],
...
function deleteBill(id) {
  return new Promise((reslove, reject) => {
    var date = new Date();
    pool.query("DELETE FROM bill WHERE idBill= ?", [id], (err, rows) => {
      if (err) {
        reject(err);
        throw err;
      } else reslove(rows);
...
function getBills(data) {
  ...
  return new Promise((reslove, reject) => {
    if (data["user"]["idRole"] == 2) {
      var query =
        "SELECT * FROM bill WHERE dateCreate BETWEEN ? AND ? And idShop=? ";
      if (status == 0 || status == 1) query += "AND status=" + status + " ";
    ...
    if (data["user"]["idRole"] == 1) {
      var query =
        "SELECT * FROM bill WHERE dateCreate BETWEEN ? AND ? And idShop=? AND idSeller=? ";
      if (status == 0 || status == 1) query += "AND status=" + status + " ";
      pool.query(
        query,
        [statDate, endDate, data["idShop"], data["user"]["idUser"]],
        (err, result) => {
          if (err) {
            console.log(err);
            reject(err);
...
function getCurrentBills(data) {
  ...
  return new Promise((reslove, reject) => {
    if (data["user"]["idRole"] == 2) {
      try {
        pool.query(
          "SELECT * FROM bill WHERE dateCreate BETWEEN ? AND ? And idShop=? AND status=0",
          [statDate, endDate, data["idShop"]],
          (err, result) => {
            if (err) {
              console.log(err);
              throw err;
