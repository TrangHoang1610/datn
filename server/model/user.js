...
function createUser(params) {
  ...
  return new Promise((reslove, reject) => {
    var date = new Date();
    pool.query(
      "INSERT INTO user (name, username, email, password,idRole, image, createDate) values(?,?,?,?,?,?,?)",
      [
        params["name"],
        params["username"],
        params["email"],
        cryptr.encrypt(params["password"]),
        params["role"],
        params["image"],
...
function updateUser(params) {
  return new Promise((reslove, reject) => {
    var date = new Date();
    pool.query(
      "UPDATE user SET name=?,username=?,email=?,password=?,image=? WHERE idUser=?",
      [
        params["name"],
        params["username"],
        params["email"],
...
function changePass(value) {
  return new Promise((reslove, reject) => {
    pool.query(
      ...
      [cryptr.encrypt(value["password"]), value["idUser"]],
      (err, rows) => {
        if (err) {
          console.log(err);
          reject(err);
...
function createPersonnel(params) {
  return new Promise((reslove, reject) => {
    ...
    console.log(params);
    pool.query(
      "INSERT INTO personnel (idPersonnel,idShop,status) values(?,?,?)",
      [params["idUser"], params["idShop"], params["status"]],
      (err, rows) => {
        if (err) {
          console.log(err);
          reject(err);
...
function getpersonnels(idShop) {
  return new Promise((reslove, reject) => {
    pool.query(
      "SELECT name, image, username, password, email, createDate, idRole,idPersonnel, idShop, status FROM personnel, user WHERE idShop= ? AND idUser=idPersonnel ",
      [idShop],
      (err, rows) => {
        if (err) reject(err);
        else {
...
function deletePersonnel(idUser) {
  return new Promise((reslove, reject) => {
    pool.query(
      "UPDATE personnel SET status=0 WHERE idPersonnel=?",
      [idUser],
      (err, rows) => {
        if (err) reject(err);
        else reslove(rows);
