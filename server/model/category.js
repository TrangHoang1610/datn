...
function insertCategory(params) {
  return new Promise((reslove, reject) => {
    pool.query(
      "INSERT INTO category( idShop, nameCategory) VALUES (?,?)",
      [params["idShop"], params["nameCategory"]],
      (err, rows) => {
        if (err) reject(err);
        else reslove(rows);
      }
...
function updateCategory(params) {
  return new Promise((reslove, reject) => {
    ...
    pool.query(
      ...
      [params["nameCategory"], params["idCategory"]],
      (err, rows) => {
        console.log(err);
...
function getListCategory(idShop) {
  return new Promise((reslove, reject) => {
    pool.query(
      "SELECT * FROM category WHERE idShop=? ORDER BY idCategory DESC",
      [idShop],
      (err, rows) => {
        if (err) reject(err);
        else reslove(rows);
