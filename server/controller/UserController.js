...
async function createUser(req, res) {
  if (req.body["image"] != null && req.body["image"] != "") {
    ...
    require("fs").writeFile(path + imageName, base64Data, "base64", function (
      err
    ) {
  ...
  if (result.length > 0) {
    if (result[0] != undefined) {
      if (cryptr.decrypt(result[0]["password"]) == req.body["password"]) {
        ...
        if (result[0]["idRole"] == 2) {
          ...
          if (result[0]["idRole"] == 1) {
            ...
            if (personnel != undefined) {
              ...
              res
                .status(400)
                .json({ status: 400, message: "Đăng nhập không thành công!" });
        ...
        console.log(
          cryptr.decrypt(result[0]["password"]) == req.body["password"]
        );
...
async function getpersonnels(req, res) {
  userModel
    .getpersonnels(req.query.idShop)
    .then((value) => {
      console.log(value);
...
async function updateUser(req, res) {
  if (
    req.body["image"] != null &&
    req.body["image"] != "" &&
    !req.body["image"].includes("/upload")
  ) {
    var path = "./public";
    var optionalObj = { fileName: "image" + Date.now(), type: "png" };
    var base64Data = req.body["image"].replace(/^data:image\/png;base64,/, "");
...
function deletePersonnel(req, res) {
  userModel
    .deletePersonnel(req.body["idUser"])
    .then((value) => {
      if (value.affectedRows > 0) {
